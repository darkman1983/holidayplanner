...
class ErrorHandler {
  ...
  function ErrorHandler( $ip = 0, $show_user = 1, $show_developer = 1, $email = NULL, $log_file = NULL ) {
    $this->ip = $ip;
    ...
    $this->error_names = array ('E_ERROR','E_WARNING','E_PARSE','E_NOTICE','E_CORE_ERROR','E_CORE_WARNING','E_COMPILE_ERROR','E_COMPILE_WARNING','E_USER_ERROR','E_USER_WARNING','E_USER_NOTICE','E_STRICT','E_RECOVERABLE_ERROR' );
  ...
  function handler( $errno, $errstr, $errfile, $errline, $errcontext ) {
    ...
    if ( $this->show_user ) $this->error_msg_basic ( );
...
$this->error_msg_detailed ( );
  ...
  function log_error_msg( ) {
    ...
    if ( ! $fp = fopen ( $this->log_file, 'a+' ) ) $this->log_message = "Could not open/create file: $this->log_file to log error.";
    $log_error = true;
    
    if ( ! fwrite ( $fp, $message ) ) $this->log_message = "Could not log error to file: $this->log_file. Write Error.";
    $log_error = true;
    
    if ( ! $this->log_message ) $this->log_message = "Error was logged to file: $this->log_file.";
